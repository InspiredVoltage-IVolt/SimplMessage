...
namespace SimplSockets
{
    ...
    public sealed class AsyncManualResetEvent
    {
        ...
        private Task WaitAsync(CancellationToken cancellationToken)
        {
            ...
            {
                var ret = _tcs.Task;
                cancellationToken.Register(() => _tcs.TrySetCanceled(), useSynchronizationContext: false);
                //Enlightenment.Trace.AsyncManualResetEvent_Wait(this, ret);
                return ret;
            }
        }
        ...
        /// Asynchronously waits for this event to be set.
        /// </summary>
        public async Task<bool> WaitAsyncR(CancellationToken cancellationToken)
        {
            var task = WaitAsync();
            await task;
        ...
        private Task WaitAsync(int timeOut)
        {
            lock (_sync)
            {
                var ret = _tcs.Task;
        ...
        /// Asynchronously waits for this event to be set.
        /// </summary>
        public async Task<bool> WaitAsyncR(int timeOut)
        {
            var task = WaitAsync();
            await task;
            return !task.IsCanceled;
        ...
        {
            var ret = WaitAsync();
            if (ret.IsCompleted) { return !ret.IsCanceled; }
            ret.Wait(cancellationToken);
            return !ret.IsCanceled;
        }
